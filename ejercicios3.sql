...
-- 1
select c.nombre_cliente, e.nombre, e.apellido1 from cliente c left join empleado e
on c.codigo_empleado_rep_ventas = e.codigo_empleado;

-- 2
select c.nombre_cliente, e.nombre, e.apellido1
from pago left join cliente c on pago.codigo_cliente = c.codigo_cliente
...
from pago p left join cliente c on p.codigo_cliente = c.codigo_cliente 
left join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
...
from oficina o right join empleado e on o.codigo_oficina = e.codigo_oficina
...
-- 8
select concat(a.nombre, ' ', a.apellido1, ' ', a.codigo_jefe) as empleado, concat(b.nombre, ' ', b.apellido1, ' ', b.codigo_empleado) as jefe 
from empleado a join empleado b on b.codigo_empleado = a.codigo_jefe;

-- 9
select concat(a.nombre, ' ', a.apellido1, ' ', a.codigo_jefe) as empleado, concat(b.nombre, ' ', b.apellido1, ' ', b.codigo_empleado, ' ', b.codigo_jefe) as jefe, 
concat(c.nombre, ' ', c.apellido1, ' ', c.codigo_empleado) as jefe_del_jefe
from empleado a join empleado b on a.codigo_jefe = b.codigo_empleado 
join empleado c on b.codigo_jefe = c.codigo_empleado;

-- 10
select c.nombre_cliente from cliente c right join pedido p 
on p.codigo_cliente = c.codigo_cliente
where p.fecha_entrega >= p.fecha_esperada;

-- 11
select cli.nombre_cliente, prod.gama
from cliente cli left join pedido ped on cli.codigo_cliente = ped.codigo_cliente
left join detalle_pedido det_ped on ped.codigo_pedido = det_ped.codigo_pedido
left join producto prod on det_ped.codigo_producto = prod.codigo_producto
group by prod.gama;
...
-- 2
select c.nombre_cliente from cliente c left join pedido p 
on c.codigo_cliente = p.codigo_cliente
where c.codigo_cliente not in (select p.codigo_cliente from pedido p);

-- 4
select e.nombre, e.apellido1 from empleado e left join oficina o
on e.codigo_oficina = o.codigo_oficina 
where o.codigo_oficina not in (select e.codigo_oficina from empleado e);

-- 5
select e.nombre, e.apellido1 from empleado e left join cliente c 
on c.codigo_empleado_rep_ventas = e.codigo_empleado
where e.codigo_empleado not in (select c.codigo_empleado_rep_ventas from cliente c);

-- 6
select e.nombre, e.apellido1, o.ciudad, o.pais, o.region, o.codigo_postal, o.telefono 
...
from pedido p right join detalle_pedido d 
on p.codigo_pedido = p.codigo_pedido
where p.codigo_pedido not in (select d.codigo_pedido from detalle_pedido d);

-- 11
select cliente.nombre_cliente 
from pago left join cliente on pago.codigo_cliente = cliente.codigo_cliente 
where cliente.codigo_cliente not in (select pago.codigo_cliente from pago);

-- CONSULTAS RESUMEN
-- 1
select count(codigo_empleado) empleados from empleado;

-- 2
select count(codigo_cliente) clientes, pais from cliente group by pais;
...
from cliente c left join empleado e on e.codigo_empleado = c.codigo_empleado_rep_ventas
...
from cliente c right join pago p on c.codigo_cliente = p.codigo_cliente
...
-- 14
select dp.cantidad, pro.nombre from pedido p left join detalle_pedido dp on p.codigo_pedido = dp.codigo_pedido
left join producto pro on pro.codigo_producto = dp.codigo_producto
order by dp.cantidad desc limit 20;
